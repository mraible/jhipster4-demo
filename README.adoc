...
This article shows you how to build a simple blog application with https://jhipster.github.io/2017/02/04/jhipster-release-4.0.1.html[JHipster 4.0.1].
...
.Blog entries
image::static/blog-entries.png[Blog entries, 1138, scaledwidth=100%]

You might notice the gap in the top menu when you're logged in as a non-admin user. This is https://github.com/jhipster/generator-jhipster/issues/5063[a bug in JHipster 4.0.0]. You can fix it by modifying `has-authority.directive.ts` to use `hidden-xs-up` instead of the `invisible` CSS class. I'd recommend changing this in the `has-any-authority.directive.ts` file too.
...
After making this change, the gap should disappear.

.Blog entries
image::static/blog-entries-menu-fixed.png[Blog entries, 1127, scaledwidth=100%]
